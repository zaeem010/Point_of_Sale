...
{
    public class BankPaymentController : BaseController
    {
        private readonly ApplicationDbContext _db;
        public BankPaymentController(ApplicationDbContext db)
        {
            _db = db;
        }
        [HttpGet]
        [Route("/Bank-Payment/Bank-Paid-Detail")]
        public async Task<IActionResult> Index()
        {
            var Li = await _db.VoucherMaster.Where(c => c.Vtype.Equals("BPV"))
        ...
        [HttpGet]
        [Route("/Bank-Payment/Pay-Using-Bank")]
        public IActionResult Create(VoucherMaster VoucherMaster)
        {
            ...
            VoucherMaster.GeneralId = _db.VoucherMaster.Where(p => p != null && p.Vtype.Equals("BPV")).Select(p => p.GeneralId).DefaultIfEmpty().Max() + 1;
