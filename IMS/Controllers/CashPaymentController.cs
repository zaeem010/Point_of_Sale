...
namespace IMS.Controllers
{
    public class CashPaymentController : BaseController
    {
        ...
        public CashPaymentController(ApplicationDbContext db)
        {
            _db = db;
        }
        [HttpGet]
        [Route("/Cash-Payment/Cash-Paid-Detail")]
        public async Task<IActionResult> Index()
        {
            var Li = await _db.VoucherMaster.Where(c=>c.Vtype.Equals("CPV"))
        ...
        [HttpGet]
        [Route("/Cash-Payment/Pay-Using-Cash")]
        public IActionResult create(VoucherMaster VoucherMaster)
        {
            ...
            VoucherMaster.GeneralId = _db.VoucherMaster.Where(p => p != null && p.Vtype.Equals("CPV")).Select(p => p.GeneralId).DefaultIfEmpty().Max() + 1;
