...
namespace IMS.Controllers
{
    public class CashReceiptController : BaseController
    {
        ...
        public CashReceiptController(ApplicationDbContext db)
        {
            _db = db;
        }
        [HttpGet]
        [Route("/Cash-Receipt/Cash-Received-Detail")]
        public async Task<IActionResult> Index()
        {
            var Li = await _db.VoucherMaster.Where(c => c.Vtype.Equals("CRV"))
        ...
        [HttpGet]
        [Route("/Cash-Receipt/Recieve-In-Cash")]
        public IActionResult create(VoucherMaster VoucherMaster)
        {
            ...
            VoucherMaster.GeneralId = _db.VoucherMaster.Where(p => p != null && p.Vtype.Equals("CRV")).Select(p => p.GeneralId).DefaultIfEmpty().Max() + 1;
